...
FAST = "ffmpeg -y -i %s -loglevel error -stats -c:a copy -profile:v baseline -level 3.0 -movflags +faststart -f mp4 _tmp"
...
def transcode(orig, tmp=False, fast=True):
	...
	cmd((fast and FAST or TRANS)%(orig,))
