...
CT.panel = {
	...
	"slider": function(data, triggerNode, parentNode, ricon, activeClass, content, resize, hovs, clearz, ngen, tfirst) {
		...
		node.add = function(d, trigger) {
			...
			if ((trigger === true) || (tfirst && (trigger == 0)))
				t.trigger();
			return n;
		};
		data.forEach(node.add);
		resize && CT.on("resize", function() {
			CT.dom.each(node, function(n, i) {
				n.style.top = (node.clientHeight * i) + "px";
			});
			node._cur && node.slide(node._cur);
		});
		return node;
	},
