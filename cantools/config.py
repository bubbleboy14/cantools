...
for key, val in items:
	...
	if key == "DB":
		...
		target = key.lower()
		c = config
		if target in ["pubsub_botnames", "log_allow", "geo_user_geonames", "geo_user_google", "admin_contacts", "admin_reportees", "web_rollz", "admin_monitor_geo", "build_exclude", "build_include", "web_whitelist", "admin_whitelist", "web_blacklist", "admin_blacklist"]: # TODO: use default list detection instead
			val = val.split("|")
			if target == "web_rollz":
				rollz = {}
				for v in val:
